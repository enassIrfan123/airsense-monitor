...
export function useAirQualityAlerts(
  ...
  useEffect(() => {
    ...
    // PM2.5 alerts
    if (
      thresholds.pm25Level === 'unhealthy' ||
      thresholds.pm25Level === 'very-unhealthy' ||
      thresholds.pm25Level === 'hazardous'
    ) {
      const key = 'pm25';
    ...
    // PM10 alerts
    if (
      thresholds.pm10Level === 'unhealthy' ||
      thresholds.pm10Level === 'very-unhealthy' ||
      thresholds.pm10Level === 'hazardous'
    ) {
      const key = 'pm10';
      if (!lastAlertRef.current[key] || now - lastAlertRef.current[key] > ALERT_COOLDOWN) {
        alerts.push({
    ...
    // O3 alerts
    if (
      thresholds.o3Level === 'unhealthy' ||
      thresholds.o3Level === 'very-unhealthy' ||
      thresholds.o3Level === 'hazardous'
    ) {
      const key = 'o3';
      if (!lastAlertRef.current[key] || now - lastAlertRef.current[key] > ALERT_COOLDOWN) {
        alerts.push({
